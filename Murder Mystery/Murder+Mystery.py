...
murder_note = """You may call me heartless, a killer, a monster, a murderer, but I'm still NOTHING compared to the 
                ...
                feed Jay's massive, massive ego. SURE you think you know him! You've seen him smiling for the cameras,
                laughing, joking, telling stories, waving his money around like a prop but off camera he was a 
                sinister beast, a cruel cruel taskmaster, he treated all of us like slaves, like cattle, like animals!
                Do you remember Lindsay, she was the first to go, he called her such horrible things that she cried 
                all night, keeping up all up, crying, crying, and more crying, he broke her with his words. I miss my 
                former cast members, all of them very much. And we had to live with him, live in his home, live in his
                power, deal with his crazy demands. AND FOR WHAT?! DID YOU KNOW THAT THE PRIZE ISN'T REAL? He never 
                intended to marry one of us! The carrot on the stick was gone, all that was left was stick, he told us
                last night that we were all a terrible terrible disappointment and none of us would ever amount to 
                anything, and that regardless of who won the contest he would never speak to any of us again! It's 
                definitely the things like this you can feel in your gut how wrong he is! Well I showed him, he got 
                what he deserved all right, I showed him, I showed him the person I am! I wasn't going to be pushed 
                around any longer, and I wasn't going to let him go on pretending that he was some saint when all he 
                was was a sick sick twisted man who deserved every bit of what he got. The fans need to know, Jay 
                Stacksby is a vile amalgamation of all things evil and bad and the world is a better place without 
                him."""
lily_trebuchet_intro = """Hi, I'm Lily Trebuchet from East Egg, Long Island. I love cats, hiking, and curling up 
                        under a warm blanket with a book. So they gave this little questionnaire to use for our 
                        bios so lets get started. What are some of my least favorite household chores? Dishes, oh
                        yes it's definitely the dishes, I just hate doing them, don't you? Who is your favorite 
                        actor and why? Hmm, that's a hard one, but I think recently I'll have to go with Michael B.
                        Jordan, every bit of that man is handsome, HANDSOME! Do you remember seeing him shirtless?
                        I can't believe what he does for the cameras! Okay okay next question, what is your perfect
                        date? Well it starts with a nice dinner at a delicious but small restaurant, you know like 
                        one of those places where the owner is in the back and comes out to talk to you and ask you
                        how your meal was. My favorite form of art? Another hard one, but I think I'll have to go 
                        with music, music you can feel in your whole body and it is electrifying and best of all, 
                        you can dance to it! Okay final question, let's see, What are three things you cannot live
                        without? Well first off, my beautiful, beautiful cat Jerry, he is my heart and spirit animal.
                        Second is pasta, definitely pasta, and the third I think is my family, I love all of them 
                        very much and they support me in everything I do. I know Jay Stacksby is a handsome man and
                        all of us want to be the first to walk down the aisle with him, but I think he might truly 
...
myrtle_beech_intro = """Salutations. My name? Myrtle. Myrtle Beech. I am a woman of simple tastes. I enjoy reading, 
                        ...
                        that one does not make and keep money without having a modicum of discipline. As such, I am 
                        hopeful. I will now list three things I cannot live without. Water. Emery boards. Dogs. Thank
                        you for the opportunity to introduce myself. I look forward to the competition."""
gregg_t_fishy_intro = """A most good day to you all, I am Gregg T. Fishy, of the Fishy Enterprise fortune. I am 37 
                        years young, an adventurous spirit and I've never lost my sense of childlike wonder. I do love
                        to be in the backyard gardening and I have the most extraordinary time when I'm fishing. 
                        Fishing for what, you might find yourself asking? Why, I happen to always be fishing for 
                        compliments of course! I have a stunning pair of radiant blue eyes that will pierce the soul
                        of anyone who dare gaze upon my countenance. I quite enjoy going on long jaunts through garden
                        paths and short walks through greenhouses. I hope that Jay will be as absolutely interesting 
                        as he appears on the television, I find that he has some of the most curious tastes in style 
                        and humor. When I'm out and about I quite enjoy hearing tales that instill in my heart of 
                        hearts the fascination that beguiles my every day life, every fiber of my being scintillates
                        and vascillates with extreme pleasure during one of these charming anecdotes and significantly
...
def get_average_sentence_length(text):
    # Finds the average sentence length
    numsentences = 0
    totallength = 0
    sentence_lengths = []
    noexclam = text.replace("!", ".")
    noquest = noexclam.replace("?", ".")
    sentences = noquest.split(".")
    for sentence in sentences:
        numsentences = numsentences + 1
        sentence_lengths.append(len(sentence))
    avgsentence = sum(sentence_lengths)/numsentences
    return avgsentence
...
class TextSample:
    # A class that creates and assigns various attributes that describes the author's text sample
...
def prepare_text(text):
    # Replaces uppercase letters with lowercase and removes all the punctuation
    # Ignore apostrophes to preseve use of conjunctions
    lowertext = text.lower()
    nocomma = lowertext.replace(",", "")
    nopoint = nocomma.replace(".", "")
    nonewline = nocomma.replace("\n", "")
    words = nonewline.split()
    return words
...
def build_frequency_table(corpus):
    # Creates a frequency table from a list of words
    frequency_table = {}
    for value in corpus:
        frequency_table[value] = corpus.count(value)
...
def ngram_creator(text_list):
    ...
    for i in range(1, len(text_list)):
        ngramlist.append(text_list[i-1] + " " + text_list[i])
    return ngramlist
...
def frequency_comparison(table1, table2):
    # Compares the frequency of two tables and scores them on the frequency
    ...
    for key in table2:
        if key not in table1:
            appearances = appearances + table2[key]
    return (mutual_appearances/(mutual_appearances + appearances))
...
def percent_difference(value1, value2):
    # Finds the percent difference between two values
    percent = (abs(value1 - value2))/((value1 + value2)/2)
    return percent
...
def find_text_similarity(TextSample1, TextSample2):
    # Finds the percentage match of a TextSample2 against TextSample1 by calculating the average similarity between 
    # the sentence length, word count and n-grams
    sentence_length_difference = percent_difference(TextSample1.average_sentence_length, 
                                                    TextSample2.average_sentence_length)
    sentence_length_similarity = abs(1 - sentence_length_difference)
    word_count_similarity = frequency_comparison(TextSample1.word_count_frequency, 
                                                 TextSample2.word_count_frequency)
...
for contestant in contestants:
    print("\nTesting " + contestant.author + ":")
    print("Reading...")
    print("Analysing...")
    print("Validating...")
    result = find_text_similarity(murderer_sample, contestant)
    results.append(result)
    print("Result: " + str(result))

max_result = max(results)
max_index = results.index(max_result)
print("\n" + contestants[max_index].author + " has the largest match! Arrest them now!")
